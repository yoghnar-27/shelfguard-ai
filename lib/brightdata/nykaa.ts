...
/**
 * Server-Only Nykaa Scraper Adapter via Bright Data Datasets v3 API
 */

if (typeof window !== "undefined") {
  throw new Error("Nykaa scraper module cannot be imported in client-side code.")
}
...
export function isNykaaUrl(url: string): boolean {
  if (!url || typeof url !== "string") return false
  try {
    const parsed = new URL(url.trim())
    return parsed.hostname.toLowerCase().includes("nykaa.com")
  } catch {
    return false
  }
}

export function mapNykaaToMarketplaceProduct(
...
export async function scrapeNykaaProduct(url: string): Promise<NykaaScrapeResult> {
  const apiKey = cleanEnvString(process.env.BRIGHTDATA_API_KEY)
  const datasetId = cleanEnvString(process.env.NYKAA_DATASET_ID)
  const baseUrl = cleanEnvString(
    process.env.BRIGHTDATA_API_BASE_URL || "https://api.brightdata.com"
  )

  const trimmedUrl = url.trim()
  if (!isNykaaUrl(trimmedUrl)) {
    return {
      success: false,
      source: "unconnected",
      product: {
        ...
        productUrl: trimmedUrl || "https://www.nykaa.com",
        lastChecked: new Date().toISOString(),
        isLive: false,
      },
      error: "Invalid Nykaa product URL. Must contain 'nykaa.com'.",
    }
  }
  ...
  if (!apiKey || !datasetId) {
    return {
      ...
      error: "Nykaa dataset integration not configured in environment.",
    }
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Nykaa Datasets API HTTP ${response.status}: ${bodyText}`)
    }
    ...
    if (!rawRecords.length || !rawRecords[0]) {
      throw new Error("Nykaa Datasets API returned no records.")
    }

    const mapped = mapNykaaToMarketplaceProduct(rawRecords[0], trimmedUrl)
    if (mapped.price <= 0) {
      throw new Error("Nykaa record contained no valid price > 0.")
    }
